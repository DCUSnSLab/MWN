...
class WeatherAlertSystem:
    ...
    def get_market_thresholds(self, market: Market) -> Dict[str, Any]:
        ...
        return {
            ...
            'forecast_hours': alert_conditions.get('forecast_hours', 24)
        }
    ...
    def check_all_weather_conditions_for_market(self, market: Market, hours: int = None) -> Dict[str, Any]:
        ...
        # forecast_hours가 파라미터로 전달되지 않은 경우 시장 설정값 사용
        hours = hours or thresholds.get('forecast_hours', self.forecast_hours)
    ...
    def _is_duplicate_alert(self, market_id: int, alert_type: str, forecast_time: str) -> bool:
        """
        중복 알림 여부 확인
        같은 시장, 같은 알림 타입, 같은 예보 시간에 대해 최근 6시간 이내에 알림을 보냈는지 확인
        """
        try:
            from datetime import timedelta
            from app import db
            # MarketAlarmLog 모델은 위에서 import 되었음
            ...
            query = MarketAlarmLog.query.filter(
                MarketAlarmLog.market_id == market_id,
                MarketAlarmLog.alert_type == alert_type,
                MarketAlarmLog.created_at > cutoff_time
            )
            
            # forecast_time이 같거나 (같은 이벤트), 
            # forecast_time이 없더라도 최근에 보냈으면 중복으로 간주 (단순 알림 빈도 제한)
            # 여기서는 'forecast_time'(예보된 시간)이 같다면 중복으로 봄
            if forecast_time:
                query = query.filter(MarketAlarmLog.forecast_time == forecast_time)
            
            exists = query.first() is not None
            
            if exists:
                logger.info(f"중복 알림 감지: 시장 {market_id}, 타입 {alert_type}, 시간 {forecast_time}")
            
            return exists

        except Exception as e:
            logger.error(f"중복 알림 확인 중 오류: {e}")
            return False  # 오류 시에는 알림을 보내는 쪽으로 처리 (안전하게)
    ...
    def send_weather_alert_to_users(self, market: Market, weather_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if alerts.get('high_temp'):
                ...
                forecast_time = alerts['high_temp'][0].get('time_str')
            elif alerts.get('low_temp'):
                alert_type = 'low_temp'
                forecast_time = alerts['low_temp'][0].get('time_str')
            elif alerts.get('strong_wind'):
                alert_type = 'strong_wind'
                forecast_time = alerts['strong_wind'][0].get('time_str')
            elif alerts.get('snow'):
                alert_type = 'snow'
                forecast_time = alerts['snow'][0].get('time_str')
            elif alerts.get('rain'):
                alert_type = 'rain'
                forecast_time = alerts['rain'][0].get('time_str')
                
            # 중복 체크 (알림 타입이 식별된 경우에만)
            if alert_type and self._is_duplicate_alert(market.id, alert_type, forecast_time):
                return {
                    'success': True,
                    'message': f'{market.name} 중복 알림으로 전송 생략 ({alert_type}, {forecast_time})',
                    'sent_count': 0,
                    'skipped': True
                }
